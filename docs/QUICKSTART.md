...
（バージョン `1.0.7` は [packages/mcp/package.json](../packages/mcp/package.json) の `version` に合わせて更新してください。）

2. 表示に従ってサーバー URL（例: 上の `https://your-app.onrender.com`）と、必要なら API キー・Client ID を入力。
...
- **API エラー**  
  → `.env` の `OPENAI_API_KEY` と `SERPER_API_KEY`（必要なら `SERPAPI_API_KEY` も） が正しく設定されているか確認。
