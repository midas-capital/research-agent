...
1. **MCP のセットアップ（初回のみ）**
   ...
   ターミナルで次を実行し、表示に従ってサーバー URL（と必要なら API キー・Client ID）を入力します。**npm パッケージ `research-agent-mcp` のバージョンは [packages/mcp/package.json](packages/mcp/package.json) の `version` と一致**（現行 **1.0.7**）を前提にしています。

   ```bash
   npx -y research-agent-mcp@1.0.7 research-agent-mcp-setup --url=https://your-app.onrender.com
   ```

   最新版を常に取りたい場合は `@1.0.7` を `@latest` に置き換えても構いません。
...
│       └── ...
├── packages/mcp/           # npm パッケージ `research-agent-mcp`（バージョンは同梱 package.json の `version`／現行 **1.0.7**）
