...
async function fetchRunState(runId: string): Promise<RunState | null> {
  ...
  console.log("[research-agent-mcp] fetchRunState", {
    runId,
    url,
  ...
  if (!res.ok) {
    ...
    console.error("[research-agent-mcp] fetchRunState failed", {
      runId,
      status: res.status,
      url,
      preview,
  ...
  console.log("[research-agent-mcp] fetchRunState ok", { runId, status: data.status });
  return data;
}

async function fetchLatestRunId(): Promise<string | null> {
  const url = `${SERVER_URL.replace(/\/$/, "")}/api/runs/latest`;
  console.log("[research-agent-mcp] fetchLatestRunId", {
    url,
    hasApiKey: Boolean(SERVER_API_KEY),
    hasClientId: Boolean(SERVER_CLIENT_ID),
  });
  ...
  if (!res.ok) {
    ...
    console.error("[research-agent-mcp] fetchLatestRunId failed", {
      status: res.status,
      url,
      preview,
    });
  ...
  console.log("[research-agent-mcp] fetchLatestRunId ok", { runId: data.runId ?? null });
...
server.registerTool(
  ...
  async ({ query }) => {
    ...
    console.log("[research-agent-mcp] tool search_cases called", {
      ...
      hasApiKey: Boolean(SERVER_API_KEY),
      hasClientId: Boolean(SERVER_CLIENT_ID),
      serverUrlPresent: Boolean(SERVER_URL),
    });
